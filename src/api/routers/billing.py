...
from src.common.enums import SubscriptionLevel
...
def plans() -> JSONResponse:
    return JSONResponse(
        content={
            str(level.value): float(plan.price) for (level, plan) in PLANS.items()
        },
        status_code=200,
    )
...
async def subscription(
    ...
    message = {
        "success": True,
        "level": active.level.value,
        "status": active.status.value,
        "start_at": str(active.start_at) if active.start_at is not None else None,
        "end_at": str(active.end_at) if active.end_at is not None else None,
    }
...
async def checkout(
    ...
    return JSONResponse(
        content={"success": True, "confirm_url": confirm_url}, status_code=200
    )
