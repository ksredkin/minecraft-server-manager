...
from src.api.services.backup_service import get_backup_service, BackupService

backups_router = APIRouter(prefix="/backups")

@backups_router.get("/", description="Просмотреть список резервных копий.")
def get_backups(backup_service: BackupService = Depends(get_backup_service)) -> JSONResponse:
...
def create_backup(backup_service: BackupService = Depends(get_backup_service)) -> JSONResponse:
