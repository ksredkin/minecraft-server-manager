...
def test_get_safe_path(tmp_path: Path) -> None:
    ...
    assert (
        file_service._get_safe_path(server, "folder/file.txt")
...
def test_write_file(tmp_path: Path) -> None:
    ...
    assert file_service.write_file(server, not_existing_file.name, "123") == FileItem(
        not_existing_file.name,
        not_existing_file,
        not_existing_file.stat().st_size,
        "123",
    )
...
def test_update_file(tmp_path: Path) -> None:
    ...
    assert file_service.update_file(
        server, file.name, new_content="new content"
    ) == FileItem(file.name, file, file.stat().st_size, "new content")
    assert file_service.update_file(
        server, file.name, new_path=new_file_path.name
    ) == FileItem(
        new_file_path.name, new_file_path, new_file_path.stat().st_size, "new content"
    )
    with pytest.raises(FileServiceError):
...
def test_update_folder(tmp_path: Path) -> None:
    ...
    assert file_service.update_folder(
        server, folder.name, new_path=new_folder_path.name
    ) == FolderItem(new_folder_path.name, new_folder_path, [])
...
def test_get_item_returns_none_for_file_without_item_path(tmp_path: Path) -> None:
    ...
    with mock.patch.object(file_service, "_get_safe_path", return_value=file):
        with pytest.raises(FileServiceError):
            file_service.get_item(object(), None)  # type: ignore
...
def test_create_folder(tmp_path: Path) -> None:
    ...
    assert file_service.create_folder(server, new_folder_path.name) == FolderItem(
        new_folder_path.name, new_folder_path, []
    )
...
def test_get_file_item_returns_item_without_content_for_binary_file(
    tmp_path: Path,
) -> None:
    file_service = FileService()
    server_folder = tmp_path / "server"
